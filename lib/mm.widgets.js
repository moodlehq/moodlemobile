...
MM.widgets = {
    ...
    renderSpinner: function(el) {
        var tpl = '\
        <p style="float:left"><label for="<%= id %>-text"><%= label %></label>\
        <input type="text" value = "  '+ MM.getConfig(el.id) +' " id="<%= id %>-text" style="border: 0; color: #f6931f; font-weight: bold;" readonly="readonly"/>\
        </p><div style="float: right; text-align: right" id="<%= id %>">\
        <button class="plus"  style="width: 24px; height: 24px; padding: 0px">+</button>\
        <button class="minus" style="width: 24px; height: 24px; padding: 0px">-</button>\
        </div>';
        return MM.tpl.render(tpl, el);
    },
