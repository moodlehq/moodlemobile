...
MM.panels = {
	...
	menuShow: function(show, settings) {
		...
		$('#panel-left, #panel-center, #panel-right, .header-main').css(
			"-webkit-transition", ""
		);
		...
		if (!MM.panels.menuStatus) {
			...
			if (!settings.animate) {
				...
				$('#panel-center').css(
					'left', sizes.center.left).css('width', sizes.center.width
				);
				$('#panel-right').css(
					'left', sizes.right.left).css('width', sizes.right.width
				);
			...
			if (MM.getConfig("dev_css3transitions")) {
				...
				$('#panel-left').css('display', 'block');
				$('#panel-center, #panel-right, .header-main').css(
	...
	fixPanelsSize: function() {
		...
		$(".header-main").css("width", MM.panels.sizes.twoPanels.top);
		$(".header-main").css("left",  MM.panels.sizes.twoPanels.top);
	},
	...
	resizePanels: function() {
		...
		// Three panel view
			...
			$("#panel-right").css("width", MM.panels.sizes.threePanels.right);
			$(".header-main").css("width", MM.panels.sizes.threePanels.top);
		}
	}
};