...
var MM = {
    ...
    setInComputerState: function() {
        MM.inComputer = navigator.userAgent.indexOf('Chrome') >= 0 ||
                        navigator.userAgent.indexOf('Safari') >= 2 ||
    ...
    /**
     * Test ajax connection and forward any errors to the user.
     */
    checkAjax: function() {
    ...
    loadCourses: function(courses) {
        ...
        values = {
            user: {fullname: MM.site.get('fullname'), profileimageurl: MM.site.get('userpictureurl')},
    ...
    /**
     * Save site success handler.
     * @param {object} json The JSON response. 
     */
    loginSuccessHandler: function(json) {
    ...
    /**
     * Save site error handler.
     * @param {object} json The JSON response. 
     */
    loginErrorHandler: function(xhr,textStatus, errorThrown) {
        // On an error, we try http instead https if possible.
        if (siteurl.indexOf("https://") === 0) {
            MM.saveSite(username, password, siteurl.replace("https://", "http://"));
            return;
    ...
    moodleUploadFile: function(data, fileOptions, successCallBack, errorCallBack, presets) {
        ...
        if (!MM.deviceConnected()) handleDisconnectedFileUpload(data, fileOptions);
    ...
    wsSyncUpload: function(sync) {
        ...
        ft.upload(
            sync.data, 
            M.config.current_site.siteurl + '/webservice/upload.php',
            function() {
    ...
    handleExternalLinks: function(selector) {
        ...
        $(selector).bind(MM.clickType, externalLinkClickHandler);
    },
