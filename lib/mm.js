...
var MM = {
    ...
    init: function(config) {
        ...
        $.ajaxSetup({"error":function(XMLHttpRequest,textStatus, errorThrown) {
            MM.popErrorMessage("Error connecting remote syte\n" + textStatus);
        }});
    ...
    loadRoutes: function() {
        var routes = [
            ["participants/:courseId", "participants", this.showParticipants],
            ["participant/:courseId/:userId", "participants", this.showParticipant],
            ["settings", "settings", this.displaySettings]
        ];
    ...
    showParticipants: function(courseId) {
        ...
        MM.moodleWSCall('moodle_user_get_users_by_courseid', data, function(users) {
            ...
            var html = _.template($("#participants_template").html(), tpl);
            MM.panels.html('center', html);
            // Load the first user
            if (MM.deviceType == "tablet") {
                MM.showParticipant(courseId, users.shift().id);
            }
        });
    },
    ...
    showParticipant: function(courseId, userId) {
        ...
        MM.moodleWSCall('moodle_user_get_course_participants_by_id', data, function(user) {
            console.log(user);
        });
    },
    ...
    panels: {
        ...
        html: function (position, content) {
