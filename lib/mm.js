...
var MM = {
    ...
    _checkMobileAuth: function(siteURL, successCallBack, errorCallback) {
        $.ajax({
            url: siteURL + "/auth/moodlemobile/check.php",
            type:'POST',
            data:{
    ...
    _appLaunchedByURL: function(url) {
        ...
        if (signature == params[0]) {
            ...
            MM.log("Inalid signature in the URL request yours: " + params[0] + " mine: " + signature);
        }
    },
     ...
     * @param {string} originalProtocol The procol entered in the url.
     */

    checkSite: function(e, originalProtocol) {
        if (e) {
            e.preventDefault();
        }
        ...
        // Load a demo site and exits thic checking function.
        if (demoSite) {
        ...
        $.ajax({
            ...
            timeout: timeout,
            success:function() {
                // Now, check if the mobile auth plugin is configured.

                MM._checkMobileAuth(siteurl,
                    // Success.
                    function(response) {
                            ...
                            switch (code) {
                                ...
                                case 2:
                                    MM.popConfirm(MM.lang.s('logininsiterequired'), function() {
                                        var passport = "abc";
                                        var url = siteurl + "/auth/moodlemobile/launch.php?service=" + MM.config.wsservice;
            ...
            error:function(xhr, textStatus, errorThrown) {
                ...
                if (siteurl.indexOf("https://") === 0) {
                    if (originalProtocol == "http://") {
                        MM.checkSite(null, originalProtocol);
                        return;
                    }
