...
var MM = {
    ...
    loadLayout: function() {
        ...
        if (matchMedia('only screen and (min-width: 768px) and (-webkit-min-device-pixel-ratio: 1)').matches) {		
            ...
            $('#mainmenu').on('click touchend', function(){
                MM.panels.menuShow();
            });
    ...
    loadSite: function(siteId) {
        ...
        MM.db.insert("settings",{id: "current_token", name: "current_token",value: site.get("token")});
    ...
    loadRoutes: function() {
        var routes = [
            ...
            ["settings/sites/:siteid", "settings_sites_delete_site", MM.settings.showSite],
    ...
    wsSync: function () {
        ...
        if (MM.deviceConnected() && MM.getConfig("sync_on")) {
