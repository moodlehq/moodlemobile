...
var MM = {
    ...
    _openFile: function(link) {
        ...
        if (MM.inNodeWK) {
            ...
            } else if (MM.deviceOS == 'ios' && cordova && cordova.plugins.fileOpener2a) {
                cordova.plugins.fileOpener2a.open(
                    link,
                    mimetype['type'],
                    {
                        error : function(errorObj) {
                            MM.log('Error opening with fileOpener2 status: ' + errorObj.status + ' - Error message: ' + errorObj.message);
                            MM._openFileWithBrowser(file);
                        },
                        success : function () {
                            MM.log('File opened with fileOpener2');
                        }
                    }
                );
            }
