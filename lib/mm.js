        ...
        if (MM.deviceConnected() && MM.getConfig("sync_ws_on")) {
            MM.db.each("sync", function(sync){
                ...
                if (sync.type == "ws") {
                    ...
                    var path = sync.siteid + "/" + sync.mod.course + "/" + sync.mod.name + "/" + sync.mod.id + "/" + sync.mod.pos;
                    // Create Dir is async (Cordova forces it).
                    var newfile = path + "/" + filename;
                    
                    MM.log("Sync: Starting download of " + newfile);
                    MM.fs.createDir(path, function() {
                        MM.moodleDownloadFile(sync.url, newfile,
                                              function() {
                                                MM.log("Sync: Download of content finished " + newfile);
                                                MM.db.remove("sync", sync.id);
                                               },
                                               function() {
                                                MM.log("Sync: Error downloading " + newfile);
                                                });
                    });
                }
            });
