...
MM.settings = {
    ...
    showDevice: function() {
        ...
        // Navigator
        var data = ["userAgent", "platform", "appName", "appVersion"];
        for (var i in data) {
            var el = data[i];
        ...
        // MM properties
        data = ["deviceReady", "deviceType", "deviceOS", "inComputer", "webApp"];
        for (var i in data) {
            el = data[i];
        ...
        if(typeof(window.device) != "undefined") {
            data = ["name", "phonegap", "platform", "uuid", "version"];
            for (var i in data) {
                var el = data[i];
        ...
        info += '<a href="file:///sdcard/temp/BWAD.pkg">test 1</a><br><br>';
        info += '<a href="file:///sdcard/temp/BWAD.pkg" target="_blank">test 2</a>';
        
        MM.panels.show("right", '<div style="padding: 8px">' + info + '</div>');
    }
};
