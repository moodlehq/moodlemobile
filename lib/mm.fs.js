...
MM.fs = {
    ...
    getRoot: function() {
        if (!MM.fs.fileSystemRoot) {
            MM.fs.loadFS(function() {
                MM.fs.fileSystemRoot.nativeURL
            });
        } else {
            var path = MM.fs.fileSystemRoot.nativeURL;
        ...
        MM.fs.fileSystemRoot.getDirectory(
            ...
            function(entry) {
                entry.getFile(
                    ...
                    function(entryFile) {
                        successCallback(entryFile.nativeURL);
                    },
                    errorCallback
                );
            },
    ...
    createDir: function(path, successCallback, dirEntry) {
        ...
        MM.log('FS: Creating directory ' + paths[0] + 'in' + baseRoot.nativeURL);
